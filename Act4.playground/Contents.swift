...
class Post {
    ...
    func ValidateDate() -> Date{
        let _signedDate:Date? = nil
        if _signedDate == now {
            return _signedDate ?? now
        }
        else
        {
            return now
...
var user = User(name: "Pedro", image: "YourImagen")
var post = Post(text: "Hola, soy Pedro", user: user)
...
print("Texto \(post.getText()) , \(post.ValidateDate())")
